...
import com.xmutca.incubator.passport.dto.TokenResponseDto;
import com.xmutca.incubator.passport.dto.UserInfoDto;
import com.xmutca.incubator.passport.feign.MemberFeign;
import com.xmutca.incubator.passport.model.OauthClientInfo;
import com.xmutca.incubator.passport.model.OauthTokenSecret;
import com.xmutca.incubator.passport.service.OauthClientInfoService;
import com.xmutca.incubator.passport.service.OauthTokenSecretService;
import com.xmutca.incubator.passport.vo.AuthorizeRequestVo;
import com.xmutca.incubator.passport.vo.TokenRequestVo;
...
public class OauthController {
    ...
    public Object tokenForPassword(TokenRequestVo requestVo) {
        // 检查账号
        Result<UserInfoDto> result = memberFeign.getByUsernamePassword(requestVo.getUsername(), requestVo.getPassword());
        ...
        Date accessExpireTime = Date.from(localDateTime.plusSeconds(systemProperties.getPassport().getAccessTokenExpireTimeout()).atZone(ZoneId.systemDefault()).toInstant());
        Date refreshExpireTime = Date.from(localDateTime.plusSeconds(systemProperties.getPassport().getRefreshTokenExpireTimeout()).atZone(ZoneId.systemDefault()).toInstant());
        OauthTokenSecret tokenSecret = OauthTokenSecret.newInitInstance(userInfo.getId(), accessExpireTime, refreshExpireTime);
        oauthTokenSecretService.save(tokenSecret);

        // 返回令牌
        String accessToken = getToken(requestVo.getClientId(), tokenSecret.getAccessTokenId(), userInfo.getId().toString(), tokenSecret.getAccessTokenSecret(), localDateTime, tokenSecret.getAccessExpireTime());
        String refreshToken = getToken(requestVo.getClientId(), tokenSecret.getRefreshTokenId(), userInfo.getId().toString(), tokenSecret.getRefreshTokenSecret(), localDateTime, tokenSecret.getRefreshExpireTime());
        return new Result<>(new TokenResponseDto(TOKEN_TYPE, accessToken, systemProperties.getPassport().getAccessTokenExpireTimeout(), refreshToken, systemProperties.getPassport().getRefreshTokenExpireTimeout()));
    }
    ...
    private Object refreshToken(OauthTokenSecret tokenSecret, String refreshTokenId, String subject) {
        ...
        return new Result<>(new TokenResponseDto(TOKEN_TYPE, accessToken, systemProperties.getPassport().getAccessTokenExpireTimeout(), (tokenSecret.getRefreshExpireTime().getTime() - System.currentTimeMillis())/1000));
    }
